...
- Receive cleartext TCP straight into the decoder's own storage on asyncio selector event loops. `asyncio.BufferedProtocol.get_buffer()` now returns a window carved out of `IncrementalDecoder`'s fixed-capacity slab, so received bytes are no longer copied through an intermediate receive buffer before reaching the parser. The decoder tracks an explicit write offset, compacts in place, and only reallocates for a frame larger than its capacity. TLS, WebSocket, Proactor, and third-party event loops keep the existing `read()` + `feed()` path.

- **Provisional API change.** On asyncio selector event loops with cleartext TCP, `TcpTransport.connect()` now returns a `PushStreamTransport` rather than an instance of the calling class, and that transport's `read()` raises: it delivers received bytes into the decoder and is driven with `receive()` instead. A subclass of `TcpTransport` that inherits `connect()` therefore no longer gets an instance of itself on that path. `mqttium.transport` is Provisional, and no in-tree subclass exists; the supported extension point remains the transport factory seam, not subclassing. Code that subclassed `TcpTransport` should override `connect()`, and code that needs `read()` semantics should keep using `asyncio.open_connection()` behind its own transport. TLS, WebSocket, Unix sockets, Proactor and third-party loops are unaffected.
