#!/usr/bin/env python3
"""Run reproducible, sharded runtime-fuzzer campaigns without changing V1."""
...
class Shard:
    """One reconstructible half-open range within a campaign."""
...
def nightly_seed_start(rotation_index: int, seeds: int) -> int:
    """Map a monotonic workflow run number to a disjoint nightly corpus."""
    if rotation_index < 1:
        raise ValueError("rotation index must be positive")
    if seeds <= 0:
        raise ValueError("seed count must be positive")
    return NIGHTLY_BASE_SEED + (rotation_index - 1) * seeds


def build_shards(*, seed_start: int, seeds: int, shard_size: int) -> list[Shard]:
    """Partition a recorded seed range into contiguous deterministic shards."""
    if seed_start < 0 or seeds <= 0 or shard_size <= 0:
        raise ValueError("seed start must be non-negative; counts must be positive")
    return [
        Shard(index, start, min(shard_size, seed_start + seeds - start))
        for index, start in enumerate(range(seed_start, seed_start + seeds, shard_size))
    ]
...
def _parser() -> argparse.ArgumentParser:
    ...
    seeds.add_argument("--nightly-rotation-index", type=int)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.seeds <= 0 or args.steps < 12 or args.shard_size <= 0 or args.parallelism <= 0:
        raise SystemExit("seeds, shard size and parallelism must be positive; steps >= 12")
    if args.nightly_rotation_index is not None:
        seed_start = nightly_seed_start(args.nightly_rotation_index, args.seeds)
    else:
        assert args.seed_start is not None
    ...
    identity = {
        ...
        "git_sha": sha,
        "nightly_rotation_index": args.nightly_rotation_index,
        "schema": CAMPAIGN_SCHEMA,
        "seed_count": args.seeds,
        "seed_end_exclusive": seed_start + args.seeds,
        "seed_start": seed_start,
        "steps": args.steps,
    }
    environment = _environment(args.runner_kind)
    shards = build_shards(seed_start=seed_start, seeds=args.seeds, shard_size=args.shard_size)
    ...
    traces = _trace_summary(repo, seed_start, args.seeds, args.steps)
    failures = _attach_failure_context(output, identity=identity, environment=environment)
    total_operations = sum(coverage.values())
    cpu_user = sum(item["performance"]["cpu_user_seconds"] for item in summaries)
    cpu_system = sum(item["performance"]["cpu_system_seconds"] for item in summaries)
    wall = time.monotonic() - campaign_started
    family_counts = Counter(
        FAMILIES[seed % len(FAMILIES)] for seed in range(seed_start, seed_start + args.seeds)
    )
    manifest: dict[str, Any] = {
        ...
        "performance": {
            ...
            "peak_shard_rss_kib": max(
                (item["performance"]["peak_rss_kib"] for item in summaries), default=0
            ),
