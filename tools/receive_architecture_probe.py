#!/usr/bin/env python3
"""Short symmetric MQTT receive probe for the direct-ingress prototype."""
...
async def run_subscriber(args: argparse.Namespace) -> dict:
    AsyncClient = _client_class(args.arm)
    ...
    try:
        ...
        stats_method = getattr(client._transport, "receive_stats", None)
        if stats_method is not None:
            receive_stats = stats_method()
    finally:
        measuring = False
...
async def run_publisher(args: argparse.Namespace) -> dict:
    AsyncClient = _client_class("base80")
    payload = b"P" * args.payload_size
    client = AsyncClient(
        f"rx-proto-fixed-pub-{os.getpid()}",
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--arm", choices=("base80", "direct"), default="base80")
    ...
    p.add_argument("--warmup-s", type=float, default=0.3)
