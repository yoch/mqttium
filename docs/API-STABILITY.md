...
`ClientStats.protocol` remains available as a deprecated compatibility aggregate
for the alpha API. New code should use `ClientStats.outbound` and
`ClientStats.inbound`; the aggregate is derived from those owner-produced
snapshots and carries no independent state.
