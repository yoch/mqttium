...
behaviors are intentionally **not supported**. The compatibility façade is a
**Provisional, additive migration layer**: the product and Stable API remain
the native `AsyncClient`.

The facade is tested and bounded, but it is not a drop-in compatibility promise,
a performance-parity promise, or a commitment to reproduce every future Paho
feature. Extend it only in response to concrete migration demand backed by
behavioral tests.
...
The runnable [`examples/paho_compat.py`](https://github.com/yoch/mqttium/blob/main/examples/paho_compat.py) demonstrates
...
| `on_publish` assignment | Plain attribute | Property that installs and clears the inner `AsyncClient.on_publish` | The façade installs a dispatcher only while a user callback exists, avoiding native completion-dispatch work for façade users who never set `on_publish` |
