...
- Consecutive PUBACK/PUBCOMP results handed off by one ingress batch settle in
  engine order. Idle synchronous `on_publish` calls run in that pass. Async,
  reentrant or occupied delivery may share one physical callback-worker job,
  but each call still consumes logical bounded-queue capacity; insufficient
