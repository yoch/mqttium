...
class WritePump:
    ...
    def __init__(
        ...
        *,
        ...
        # At most one data eager write and one success-ACK eager write may happen
        # before the event loop regains control. A tight producer therefore seeds
        # the transport immediately, then queues the rest so the writer task can
        # coalesce them. A paced producer is re-armed on the next loop turn while
        # the writer is idle.
