...
class EffectPump:
    ...
    def collect_from_engine(self) -> None:
        ...
        if self._failing_close:
            # The scheduled flush still holds the lock while awaiting
            # transport close after a failure. Effects collected in that
            # window cannot be applied; count them as settled so drain()
            # waiters are not parked on work that will never run.
            if not self.pending:
