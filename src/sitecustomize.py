...
Hypothesis:
- keep rc7's first eager frame exactly as-is;
- when the producer has accumulated four contiguous byte frames behind it,
  remove those four from the WritePump queue and issue one non-awaiting
  vector write;
- use StreamWriter.writelines() instead of b"".join() so CPython 3.13's
  selector transport can use its scatter/gather path;
- apply the same mechanism to async publish and publish_nowait, testing whether
  producer-side coalescing can match/exceed rc7 capacity while bounding queue
  residence close to rc6 latency.
