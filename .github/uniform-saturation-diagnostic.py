"""Classify bounded-delivery/network-progress behavior; diagnostic, not product code."""
...
async def main() -> None:
    ...
    loop.set_exception_handler(
        lambda _loop, context: errors.append(
            repr(context.get("exception") or context.get("message"))
        )
    )
    client = AsyncClient(
        "uniform-saturation-diagnostic",
    ...
    async def callback(message: Message) -> None:
        ...
        try:
            await asyncio.wait_for(receipt.wait(), 0.25)
        except TimeoutError:
            if not outcome.done():
    ...
    try:
        ...
        # After the callback deadline expires the worker can release queue space;
        # allow the reader/effect path to unwind so cleanup state is observable.
        for _ in range(10):
            await asyncio.sleep(0)
        print(
            json.dumps(
                {
                    "reply_progressed_with_full_callback_queue": progressed,
                    "queue_full_seen": queue_full_seen,
                    "seen_before_cleanup": seen,
                    "callback_queue_size": client._callback_queue.qsize(),
                    "effect_pending": len(client._effect_pump.pending),
