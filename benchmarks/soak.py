...
try:
    import psutil
except ImportError:  # pragma: no cover - exercised by the benchmark entry point
...
def _resource_assessment(snapshots: list[dict[str, Any]]) -> dict[str, object]:
    ...
    violations = [
        ...
        if baseline[field] is not None and trimmed[field] != baseline[field]
    ]
...
async def run(args: argparse.Namespace) -> dict[str, object]:
    ...
    try:
        ...
        if psutil is None:
            raise RuntimeError("soak.py resource sampling requires psutil>=6")
        ...
        return {
            ...
            "published": (args.warmup_cycles + measured_cycles) * args.messages_per_cycle,
