...
def test_pipelined_qos1_batch_avoids_generic_packet(monkeypatch) -> None:
    ...
    ``_read_loop`` handles up to 256 packets before ``take_effects()``. After the
    first auto-PUBACK sits in the current effect batch, the preflight used to
    call ``PublishPacket.decode`` on every subsequent QoS 1/2 PUBLISH — copying
    the payload and undoing the specialised inbound decoder.
    """
    calls = 0
    original = PublishPacket.decode
