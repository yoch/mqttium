"""Reproduction for GitHub issue #244.

MQTT 5 §4.12.1: while CONNECTED, AUTH Continue (0x18) is a Server response
during Client-initiated re-authentication. Without a pending 0x19, it is a
Protocol Error.
"""
...
@pytest.mark.xfail(
    strict=True,
    reason="https://github.com/yoch/mqttium/issues/244 — unsolicited AUTH Continue while CONNECTED",
)
def test_unsolicited_auth_continue_while_connected_is_protocol_error() -> None:
    engine = _engine()
