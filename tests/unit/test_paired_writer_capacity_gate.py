...
def test_rc6_class_capacity_collapse_is_a_regression() -> None:
    ratio, baseline_cv, candidate_cv, invalidations, regressions = _assess(
...
def test_restored_capacity_above_95_percent_passes() -> None:
    ratio, _baseline_cv, _candidate_cv, invalidations, regressions = _assess(
        [61_000.0, 61_300.0, 60_900.0, 61_200.0],
...
def test_noisy_baseline_invalidates_measurement() -> None:
    _ratio, baseline_cv, candidate_cv, invalidations, regressions = _assess(
...
def test_noisy_candidate_invalidates_measurement() -> None:
    _ratio, baseline_cv, candidate_cv, invalidations, regressions = _assess(
        [20_000.0, 20_100.0, 19_900.0, 20_050.0],
        [18_000.0, 22_000.0, 19_000.0, 23_000.0],
    )
    ...
    assert baseline_cv < 0.05
    assert candidate_cv > 0.05
    assert invalidations == [f"qos=0: candidate CV {candidate_cv:.2%}"]
    assert regressions == []
