...
def test_same_code_control_requires_ci_to_contain_one_and_be_precise() -> None:
    stable = paired_ratio_estimate(
        [0.995, 1.005, 1.002, 0.998, 0.997, 1.003, 1.004, 0.996, 1.0, 1.0, 0.999, 1.001],
        ORDERS_12,
    )
    assert control_failures("throughput", stable, floor=0.95, ceiling=1.05) == []

    biased = paired_ratio_estimate([1.03] * 12, ORDERS_12)
    failures = control_failures("throughput", biased, floor=0.95, ceiling=1.05)
    assert any("does not contain 1.0" in failure for failure in failures)

    noisy = paired_ratio_estimate(
        [0.80, 0.90, 1.10, 1.20, 0.85, 0.95, 1.05, 1.15, 0.82, 0.92, 1.08, 1.18],
        ORDERS_12,
    )
    failures = control_failures("throughput", noisy, floor=0.95, ceiling=1.05)
...
def test_raw_arm_cv_is_diagnostic_when_abba_estimator_is_precise() -> None:
    ...
    [evaluation] = evaluate_control_payload(
        ...
        throughput_ceiling=1.05,
        ack_p50_floor=0.90,
        ack_p50_ceiling=1.10,
    )
