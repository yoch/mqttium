...
def test_write_pump_individual_capacity_failure_preserves_admitted_prefix() -> None:
    pump = WritePump(max_bytes=10, max_messages=2, on_failure=_no_failure)
    assert pump.try_enqueue(b"12345") is True

    assert pump.try_enqueue(b"123") is True
    assert pump.try_enqueue(b"456") is False

    assert pump.queued_messages == 2
    assert pump.queued_bytes == 8
    assert pump.queue.get_nowait() == b"12345"
    assert pump.queue.get_nowait() == b"123"


def test_write_pump_individual_admission_updates_exact_accounting() -> None:
    pump = WritePump(max_bytes=32, max_messages=4, on_failure=_no_failure)

    assert pump.try_enqueue(b"a") is True

    assert pump.try_enqueue((b"head", b"payload")) is True
