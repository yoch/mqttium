...
async def test_callback_cell_accounts_for_every_reply_and_ack() -> None:
    result = await _callback_sample(warmup=2, count=12, timeout=5.0)

    assert result.samples == 12
    assert result.eager_acks + result.queued_acks == 12
    assert result.eager_replies + result.queued_replies == 12
