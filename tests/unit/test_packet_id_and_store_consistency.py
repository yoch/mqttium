"""Packet-identifier, inbound-session, effect, and store consistency.

The harnesses that found them live in ``tests/fuzz/test_stateful_invariants.py``
(engine invariants, store differential) and ``tests/resilience/test_hostile_broker.py``
...
def test_inbound_qos2_duplicate_is_still_answered_with_pubrec() -> None:
    ...
    assert [e for e in effects if e.kind is EffectKind.SEND_PROTOCOL_RESPONSE], (
        "duplicate must re-send PUBREC"
    )
    _assert_accounting_exact(engine)
